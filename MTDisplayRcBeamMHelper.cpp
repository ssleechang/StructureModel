...
MTDisplayRcBeamMHelper::MTDisplayRcBeamMHelper(MSBaseBuilding* pBldg, MTReader* pReader) : MTDisplayCompoMHelper(pBldg, pReader)
{
}


MTDisplayRcBeamMHelper::~MTDisplayRcBeamMHelper(void)
{
}

bool MTDisplayRcBeamMHelper::MakeData(MTRecognizeSection* pSection, vector<MTRecognizeData*>& DataArr)
{
	...
	MSRcBeamM* pRebarM = (MSRcBeamM*)pCompoM->GetRebarM();

	CString sName, sSize;
	if(pRebarM->GetIsSubBeam())
		sName.Format(_T("%s(SubBeam)"), GetName(pSection->ma_CompoM));
	else
...
void MTDisplayRcBeamMHelper::MakeSectionDatas(MSCompoM* pCompoM, SRSectionTable* pTable, vector<MTRecognizeData*>& DataArr)
{
	MSRcBeamM* pRebarM = (MSRcBeamM*)pCompoM->GetRebarM();
...
MSRcBeamSec* MTDisplayRcBeamMHelper::GetBeamSection(MSRcBeamM* pRebarM, int nSectionCount, int SectionIdx)
{
...
void MTDisplayRcBeamMHelper::MakeSectionDatas(MSRcBeamSec* pBeamSec, SRSectionBox* pSectionBox, vector<MTRecognizeData*>& DataArr)
{
